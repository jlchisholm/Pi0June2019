...
//   --> Get detection efficiency and bring that in (done, for some energies)
//   --> Also work on converting tagger channels to photon energies (done using Phil's method)
//   --> Need to do it for specific energy points and plot those now
...
void PlotCS()
{
        ...
        for (int ch=76; ch<=267; ch++)
	{
        ...
        double Edet = GetTotDetEff(300); // we'll use 300MeV edet for now, should be specific later
        ...
        hCrossSec->Divide(hCrossSec,hTaggEff,TMath::Power(10,34),Edet);       // also divide by tagging efficiency and by detection efficiency, and convert to microbarns
	...
	hCrossSec->Draw();
        ...
        hCrossSec2->GetYaxis()->SetTitle("Total Cross Section (#mub)");
