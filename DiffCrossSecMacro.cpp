...
int GetCounts(int ke, int th, TH3* hFull_ME_3D_conv, TH3* hEmpty_ME_3D_conv, double scale)
{
        ...
        TH1D *hFull_ME_projx = hFull_ME_3D_conv->ProjectionX(Form("hME_projx_%dMeV",ke),th-1,th+1,ke-1,ke+1);
        TH1D *hEmpty_ME_projx = hEmpty_ME_3D_conv->ProjectionX(Form("hEmpty_ME_projx_%dMeV",ke),th-1+1,th,ke-1,ke+1);  // maybe these should be th, th+1 ...
	...
	// Subtract empty from full with scaling
        ...
        //Create a scaled version of the empty target histogram -- wait, why do I need this
...
TH1D* GetDXS(int ke=200)
{
    ...
    TH1F *hElastic = new TH1F("hElastic", Form("Counts of Elastic Pi0 Production for KE=%d",ke),18,0,180);
    ...
    double taggeff = hTaggEff_conv->GetBinContent(hTaggEff_conv->GetXaxis()->FindBin(ke));   // could convert and then get from energy
    ...
    double scalars = hScalars_conv->GetBinContent(hScalars_conv->GetXaxis()->FindBin(ke));   // could convert and then get from energy
    ...
    TH1D *hDXS = new TH1D("hDXS", Form("Differential Cross Section at KE=%d",ke),18,0,180);
